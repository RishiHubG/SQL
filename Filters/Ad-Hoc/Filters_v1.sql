	...
	columnId: -100= OR
	columnId:-200=AND
	columnId: -1 = Register -> Search Register Name or All Register NAmes under the framework: Search in Registers TABLE & GET THE REGISTERID RegisterID; NAME COLUMN IN _DATA
	SELECT * FROM dbo.Registers WHERE Name LIKE '%Controls%'

	columnId: -3 = Role : RoleType-> Search Name-> Get RoleTypeID & Search in ContactInst, put Framework ID filter if available
	SELECT *
	FROM dbo.RoleType RT
		INNER JOIN dbo.ContactInst CI ON CI.RoleTypeID = RT.RoleTypeID
	WHERE RT.name LIKE '%Owner%' 

	columnId: -2 = Universe -> Search Universe Name or All Universe NAmes under the framework
	SELECT * FROM dbo.Universe WHERE Name LIKE '%Library%'

	columnId: -4 = Register Path -> New Table RegisterPath: SELECT * FROM dbo.RegisterPath Search Register Path Name or All Universe Path NAmes under the framework
