...
--       Scripting of with collation added/required for swcripting SharePoint/ReportServer , or databases with non standard collations
...
--##############################################################################
 SELECT 
   @FINALSQL =  --'IF OBJECT_ID(''' + QUOTENAME(@SCHEMANAME) + '.' + QUOTENAME('<TABLENAME>') + ''') IS NOT NULL ' + @vbcrlf
              --+ 'DROP TABLE ' + QUOTENAME(@SCHEMANAME) + '.' + QUOTENAME('<TABLENAME>') + ' ' + @vbcrlf + 'GO' + @vbcrlf
              --+ 
...
--##############################################################################
  SELECT @CONSTRAINTSQLS = @CONSTRAINTSQLS 
         + CASE
             ...
             THEN @vbCrLf
                  + 'CONSTRAINT   '  COLLATE SQL_Latin1_General_CP1_CI_AS + QUOTENAME(CONCAT('<TABLENAME>_',[index_name])) + ' '
                  + CASE  
...
SELECT
  ...
  'CONSTRAINT ' 
  + QUOTENAME(CONCAT('<TABLENAME>_',[conz].[name]))
  + ' FOREIGN KEY (' 
