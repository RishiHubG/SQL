...
/****** Object:  StoredProcedure [dbo].[ParseFrameworkJSONData]    Script Date: 5/11/2022 12:03:01 PM ******/
SET ANSI_NULLS ON
GO
...
ALTER   PROCEDURE [dbo].[ParseFrameworkJSONData]
@Name VARCHAR(100),
...
DROP TABLE IF EXISTS #TMP_ALLSTEPS 
 ...
 --SELECT * FROM #TMP_ALLSTEPS WHERE StringValue='selectboxes'
 --RETURN
 ...
 WHERE ValueType='Object'
	...
	--BUILD SCHEMA FOR _DATA TABLE============================================================================================	 
	 ...
	 UPDATE #TMP_DATA
		SET DataType = CASE WHEN StringValue IN ('textfield','selectboxes','select','textarea','email','URL','phoneNumber','tags','signature','password','button','colorPicker','colored','entityLinkGrid','datagrid','checkbox','radio','tableTemplate','dynamicTable','rangecolored','entityTab','queryGrid','customTreeSelection','file','chart','selectQueryGrid','customTextEditor') THEN 'NVARCHAR' 
		...
		SET @SQL = CONCAT(@SQL,N' dbo.[', @Name,'_Data_Insert]
						...
						BEGIN
							...
							ELSE IF EXISTS(SELECT 1 FROM INSERTED) AND  EXISTS(SELECT 1 FROM DELETED) --UPDATE
							   ...
							   FROM dbo.',@FrameWorkHistTblName,' Hist INNER JOIN Inserted I ON I.ID = Hist.ID 

							 INSERT INTO dbo.',@FrameWorkHistTblName,'(<ColumnList>)
									SELECT <columnList>
									FROM INSERTED;						 
							
							SELECT @HistoryID = MAX(HistoryID) FROM dbo.',@FrameWorkHistTblName,' Hist INNER JOIN Inserted I ON I.ID = Hist.ID;
							
							--UPDATE PeriodIdentifier = 1 FOR ALL THE LATEST RECORD
							UPDATE dbo.',@FrameWorkHistTblName,'
 ...
 --==================================================================================================================================
		...
		--RETURN
 		...
 		--TABLE TO HOLD STEITEMS FROM JSON - THIS IS USED TO REMOVE FROM TABLE IF NOT AVAILABLE IN JSON
		CREATE TABLE #TBL_JSONFrameworkStepItems(FrameworkID INT, StepID INT, StepItemKey nvarchar(200))
...
--PROCESS THE STEP ITEMS ONE BY ONE
 ...
 BEGIN
		...
		SET @TableName = CONCAT('[',@Name,'_',@TemplateTableName,']')
				...
				--CHECK IF THE NEW STEP IS ALREADY AVAILABLE IN HISTORY (USING STEPNAME)=================================
						...
						SET @SQL = CONCAT('IF EXISTS (SELECT 1 FROM SYS.TABLES WHERE NAME =''',@HistStepTblName,''')', CHAR(10))
						...
						BEGIN
							...
							SET @SQL = CONCAT('SELECT TOP 1 @HistStepID = StepID FROM [',@HistStepTblName,'] WHERE FrameworkID=',
												@FrameworkID,' AND STEPNAME=''',@STEPNAME,''''
											 )
						   PRINT @SQL  
						   EXEC sp_executesql @SQL, N'@HistStepID INT OUTPUT',@HistStepID OUTPUT;

						   --STEP ITEM WAS AVAILABLE, HENCE USE THE SAME STEP ITEM ID
						   IF @HistStepID IS NOT NULL
						   BEGIN
							 SET @StepID = @HistStepID
					...
					IF @IsExistingTable = 0 AND @StepID IS NULL --1ST TIME AS NO HISTORY TABLE					 
						SELECT @StepID = ISNULL(MAX(StepID),0) + 1 FROM dbo.FrameworkSteps
					ELSE IF @IsExistingTable = 1 AND @StepID IS NULL --NOT EXISTING API KEY - NEW ONE
					BEGIN
						SET @SQL = CONCAT(' SELECT @StepID = MAX(StepID) + 1 FROM ',@TableName, CHAR(10))
						PRINT @SQL  
		...
		BEGIN
			...
			SET @TableName = CONCAT('[',@Name,'_',@TemplateTableName,']')
					...
					IF @IsExistingTable = 0 AND @StepItemID IS NULL --1ST TIME AS NO HISTORY TABLE					 
						SELECT @StepItemID = ISNULL(MAX(StepItemID),0) + 1 FROM dbo.FrameworkStepItems
					ELSE IF @IsExistingTable = 1 AND @StepItemID IS NULL --NOT EXISTING API KEY - NEW ONE
					BEGIN
						SET @SQL = CONCAT(' SELECT @StepItemID = MAX(StepItemID) + 1 FROM ',@TableName, CHAR(10))
						PRINT @SQL  
		...
		--==================================================================================================================================
				...
				END
				ELSE		
				IF @StepItemType IN ('select','radio')
				BEGIN				
 ...
 END	--END OF WHILE LOOP	
						...
						BEGIN
						  ...
						  SELECT @MaxID = MAX(StepItemID) FROM dbo.FrameworkStepItems
						   ...
						   UPDATE dbo.FrameworkStepItems
							...
							--UPDATING STEP ITEM NAME IF IT IS MODIFIED
							SET @SQL = CONCAT('UPDATE Tbl SET StepItemName = TMP.StepItemName 
											  FROM [',@Name,'_FrameworkStepItems] Tbl INNER JOIN dbo.FrameworkStepItems TMP ON TMP.FrameworkID=Tbl.FrameworkID AND Tbl.StepItemKey=TMP.StepItemKey
											  WHERE Tbl.StepItemName <> TMP.StepItemName'
											  )
						   PRINT @SQL  
						   EXEC sp_executesql @SQL						  
		...
		--**DELETE FROM STEP ITEMS IF DATA NOT AVAILABLE IN JSON************************************************************			
			 DECLARE @StepItemTbl VARCHAR(100) = CONCAT('[',@Name,'_FrameworkStepItems]');

			 IF EXISTS(SELECT 1 FROM #TBL_JSONFrameworkStepItems)
			 BEGIN
				SET @IsExistingTable = 0
				...
				BEGIN		
					...
					CREATE TABLE #TMP_StepItemIDToRemove(ID INT)

					--GET STEPITEM ID'S TO REMOVE
					SET @TableName = CONCAT('[',@Name,'_FrameworkStepItems]');
					SET @SQL = CONCAT('SELECT StepItemID FROM ',@TableName,' TBL WHERE NOT EXISTS(SELECT 1 FROM #TBL_JSONFrameworkStepItems FSI  WHERE TBL.FrameworkID = FSI.FrameworkID AND TBL.StepItemKey = FSI.StepItemKey AND TBL.StepID = FSI.StepID)', CHAR(10))				
					PRINT @SQL  

					INSERT INTO #TMP_StepItemIDToRemove(ID)
						EXEC sp_executesql @SQL;

					--REMOVE STEPITEM ID'S FROM ATTRIBUTES
					SET @TableName = CONCAT('[',@Name,'_FrameworkAttributes]');
					SET @SQL = CONCAT('DELETE TBL FROM ',@TableName,' TBL WHERE EXISTS(SELECT 1 FROM #TMP_StepItemIDToRemove WHERE ID = TBL.StepItemID)', CHAR(10))				
					PRINT @SQL  
					EXEC sp_executesql @SQL;

					--REMOVE STEPITEM ID'S FROM LOOKUPS
					SET @TableName = CONCAT('[',@Name,'_FrameworkLookups]');					
					SET @SQL = CONCAT('DELETE TBL FROM ',@TableName,' TBL WHERE EXISTS(SELECT 1 FROM #TMP_StepItemIDToRemove WHERE ID = TBL.StepItemID)', CHAR(10))				
					PRINT @SQL  
					EXEC sp_executesql @SQL;
					
					--REMOVE STEPITEM ID'S
					SET @TableName = CONCAT('[',@Name,'_FrameworkStepItems]');
					SET @SQL = CONCAT('DELETE TBL FROM ',@TableName,' TBL WHERE EXISTS(SELECT 1 FROM #TMP_StepItemIDToRemove WHERE ID = TBL.StepItemID)', CHAR(10))				
					PRINT @SQL  
					EXEC sp_executesql @SQL;		

					--REMOVE STEP IF IT HAS NO STEP ITEMS LEFT					
					SET @TableName = CONCAT('[',@Name,'_FrameworkSteps]');
					SET @SQL = CONCAT('DELETE TBL FROM ',@TableName,' TBL WHERE NOT EXISTS(SELECT 1 FROM ',@StepItemTbl,' FSI  WHERE TBL.FrameworkID = FSI.FrameworkID AND TBL.StepID = FSI.StepID)', CHAR(10))				
					PRINT @SQL  
					EXEC sp_executesql @SQL;
				END					
	...
	--**********************************************************************************************************************************	
		...
		--INSERT INTO MultiSelect_FrameworkStepItems-----------------------------------------------------------------------------------
				
				SELECT PARSENAME(ParentStep.Name,1) AS StepItemKey ,
					   PARSENAME(ParentStep.Name,2) AS StepName 
					INTO #TMP_SELBOXES
				FROM #TMP_ALLSTEPS TMP_SEL
					 INNER JOIN #TMP_ALLSTEPS ParentStep ON ParentStep.Element_ID = TMP_SEL.Parent_ID			 
				WHERE TMP_SEL.StringValue='selectboxes'
					  AND TMP_SEL.Name ='type'
			  					 
				INSERT INTO [dbo].[MultiSelect_FrameworkStepItems]
						   ([UserCreated]
						   ,[DateCreated]
						   ,[UserModified]
						   ,[DateModified]
				   ...
				   SELECT  @UserLoginID
						   ...
						   ,TMP.[StepItemKey]
						   ,FSI.OrderBy				  
				FROM #TMP_SELBOXES TMP
					 INNER JOIN dbo.FrameworkSteps FS ON FS.StepName =  TMP.StepName
					 INNER JOIN dbo.FrameworkStepItems FSI ON FSI.StepID = FS.StepID AND FSI.StepItemKey = TMP.StepItemKey
				WHERE NOT EXISTS(SELECT 1 FROM [dbo].[MultiSelect_FrameworkStepItems] 
										  WHERE [FrameworkID] = @FrameworkID AND StepID = FS.StepID AND StepItemKey= FSI.StepItemKey
							     );
		-------------------------------------------------------------------------------------------------------------------------------------------------
					...
					SET @SQL = CONCAT('IF EXISTS (SELECT 1 FROM SYS.TABLES WHERE NAME =''',@Name,'_FrameworkSteps'')', CHAR(10))
					SET @SQL = CONCAT(@SQL,' SET @IsExistingTable = 1; ', CHAR(10))
					PRINT @SQL  
					EXEC sp_executesql @SQL, N'@IsExistingTable BIT OUTPUT',@IsExistingTable OUTPUT;
				...
				BEGIN
					SET @TableName = CONCAT('[',@Name,'_FrameworkSteps]');
					SET @StepItemTbl = CONCAT('[',@Name,'_FrameworkStepItems]');
					SET @SQL = CONCAT('DELETE TBL FROM ',@TableName,' TBL WHERE NOT EXISTS(SELECT 1 FROM ',@StepItemTbl,' FSI  WHERE TBL.FrameworkID = FSI.FrameworkID AND TBL.StepID = FSI.StepID)', CHAR(10))				
					PRINT @SQL  
					EXEC sp_executesql @SQL;
				END
		...
		BEGIN			
			...
			SET @FullSchemaJSON = REPLACE(@FullSchemaJSON,'''','''''');
...
END CATCH
		...
		--DROP TEMP TABLES--------------------------------------
		 ...
		 DROP TABLE IF EXISTS #TMP
		 DROP TABLE IF EXISTS #TMP_Lookups
		 DROP TABLE IF EXISTS #TBL_JSONFrameworkStepItems
		 --------------------------------------------------------
END
