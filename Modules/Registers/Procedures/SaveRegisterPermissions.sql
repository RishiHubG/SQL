...
/****** Object:  StoredProcedure [dbo].[SaveRegisterPermissions]    Script Date: 06/06/2021 11:01:27 ******/
SET ANSI_NULLS ON
GO
...
/***************************************************************************************************
OBJECT NAME:        dbo.SaveRegisterPermissions
CREATION DATE:      2021-05-22
AUTHOR:             Rishi Nayar
DESCRIPTION:		
USAGE:          	EXEC dbo.SaveRegisterPermissions   @UserLoginID=100,
													@inputJSON=  ''
...
CREATE OR ALTER PROCEDURE [dbo].[SaveRegisterPermissions]
...
BEGIN TRY
	...
	;WITH CTE 
		...
		--BUILD PERMISSIONS FOR ADMIN=1 AND ADMINUG=2---------------------------------------------------------------------------------------
			...
			ELSE
			BEGIN
				PRINT 'No Permissions Available. Please Check JSON!'
				RETURN
			END
	...
	---------------------------------------------------------------------------------------------------------------------------
		...
		-- AS WE  ARE GETTING ERRO with the below if userid id not replaced - need to be fixed
		 DELETE from #TMP_UpdateStmt where ParentID not in (SELECT ParentID FROM #TMP_Users)
