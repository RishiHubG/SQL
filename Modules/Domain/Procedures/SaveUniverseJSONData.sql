...
BEGIN TRY
	...
	BEGIN
		...
		DECLARE @UpdWhereClauseStmt VARCHAR(MAX) = CONCAT(' WHERE EntityID=',@UniverseID, CHAR(10), ' AND ContactID=<ContactID>', CHAR(10), ' AND RoleTypeID=<RoleTypeID>')
		...
		UPDATE	TMP 
			SET UpdString =  REPLACE(REPLACE( REPLACE(UpdString,'<EntityID>',@UniverseID),'<ContactID>',ContactID),'<RoleTypeID>',RoleTypeID)
		...
		UPDATE #TMP_ContactInst
			SET TableInsert = REPLACE(REPLACE( REPLACE(TableInsert,'<EntityID>',@UniverseID),'<ContactID>',ContactID),'<RoleTypeID>',RoleTypeID)
