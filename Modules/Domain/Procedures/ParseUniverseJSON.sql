		...
		--==========================================================================================================================================================
			...
			*/
			...
			--RETURN
				...
				BEGIN
					SET @SQL = CONCAT(N' ALTER TABLE dbo.UniversePropertiesXref_Data ADD', CHAR(10), @DataCols, ' NULL ',CHAR(10))
					PRINT @SQL	
					EXEC sp_executesql @SQL	

					--CREATE _DATA_HISTORY TABLE
					SET @SQL = CONCAT(N' ALTER TABLE dbo.UniversePropertiesXref_Data_history ADD', CHAR(10), @DataCols, ' NULL ',CHAR(10))
					PRINT @SQL	
					EXEC sp_executesql @SQL	
					...
					FROM sys.dm_exec_describe_first_result_set(N'SELECT * FROM dbo.UniversePropertiesXref_Data' , NULL, 1)
					...
					/*
					IF EXISTS(SELECT 1 FROM SYS.triggers WHERE NAME ='UniversePropertiesXref_Data_Update')						
						SET @SQL = N'ALTER TRIGGER '
					ELSE
						SET @SQL = N'CREATE TRIGGER '
					...
					SET @SQL = CONCAT(@SQL,N' dbo.UniversePropertiesXref_Data_Update
									   ON  dbo.UniversePropertiesXref_Data
									   AFTER UPDATE
									AS 
									BEGIN
										...
										UPDATE Hist
											SET <columnList>
										FROM dbo.UniversePropertiesXref_Data_history Hist
